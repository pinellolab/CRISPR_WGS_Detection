...
def compute_vcf(genome, outfile):
    ...
    code = subprocess.call(
        "%s -p %s_D -r %s -R hg38 -d 20131217 -G" % (PINDEL2VCF, outfile, genome),
        shell=True,
    )
    if code != 0:
        raise OSError("An error occurred while computing %s_D VCF" % (outfile))
    code = subprocess.call(
        "%s -p %s_SI -r %s -R hg38 -d 20131217 -G" % (PINDEL2VCF, outfile, genome),
        shell=True,
    )
    if code != 0:
...
def run_pindel(
    tumor_bam, tumor_sample, normal_bam, normal_sample, targets, outdir, genome, threads
):
    ...
    for target in targets:
        ...
        code = subprocess.call(
            "%s -f %s -i %s -c %s -T %d -o %s"
            % (PINDEL, genome, configfile, target, threads, outfile),
            shell=True,
        )
        if code != 0:
